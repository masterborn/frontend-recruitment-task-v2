...
The `ProductConfigurator` component is intentionally monolithic—this is a legacy decision that Marcus (previous dev) made for "simplicity." In practice, it probably should be broken into smaller components, but that's a larger refactor for another time.
